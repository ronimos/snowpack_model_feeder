...
There is a second valid reading of the same result worth surfacing, because the Little Professor dataset uniquely supports both directions of interpretation. Operationally, the Mayer model was applied exactly as designed — one column of snow per cluster, six per-layer features, P_unstable computed independently at each location. The model is answering its own profile-stability question correctly at every point on the slope; it returns "this profile contains an unstable basal facet layer" at 99.9% of clusters, which is the right answer for the question being asked. What changes is the use we put those answers to. By aggregating ~6,500 of them into a spatial map and asking whether the pattern picks out the release area, we are implicitly testing whether the six features Mayer uses vary spatially in a way that tracks triggering — a question the model was never designed to answer. The model isn't being asked the wrong question; it's being asked its own question across a slope where the spatial information needed for trigger prediction may not live in any individual profile's features. Both readings — "different questions" and "same question, different aggregation purpose" — point to the same conclusion: a profile-stability classifier and a spatial-gradient model are complementary, not competing, and the result at Little Professor doesn't decide between them.
...
There is also a real possibility that with continental-snowpack training data and more validation events, the model would discriminate more sharply even on the spatial question. The training set is heavily weighted toward Swiss profiles with surface hoar, decomposing storm snow, and surface-hoar-dominated weak layers. Colorado basal faceting produces feature distributions the model has seen less of, and the calibrated thresholds (P ≥ 0.77 for "poor," P ≥ 0.92 for "very poor") were tuned to Alpine observations. A version of this RF trained on a continental dataset, or a Colorado-specific dataset of triggered vs. non-triggered profiles at the cluster scale, might do considerably better. Until that work is done, the Mayer model on continental snow should be read as "evaluation incomplete, needs more validation on similar situations" rather than as a definitive assessment in either direction.
